...
//	Ошибка: Ошибка при вызове метода контекста (Заблокировать) по причине:
//	Использование блокировки допустимо только внутри транзакции в режиме управляемых блокировок! 
//
//	Версия платформы 8.3.27.1688
...
//	Блокировка.Заблокировать();
	...
	ВыборкаДТ = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	
	Пока ВыборкаДТ.Следующий() Цикл
		
		Нехватка = ВыборкаДТ.КоличествоДок - ВыборкаДТ.КоличествоОстаток;
		...
		Если НЕ Отказ Тогда
			ОсталосьСписать = ВыборкаДТ.КоличествоДок;
			ВыборкаДетальныеЗаписи = ВыборкаДТ.Выбрать();
